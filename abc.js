...
fire.addEventListener('click', start, false);
     ...
     function start() {
        if(isLeft){
			...
			if(sniper){
				rad = launcht2.value * Math.PI / 180;
				vox = -(launchv2.value * Math.cos(rad));
				voy = launchv2.value * Math.sin(rad);
				moveLogic = setInterval(mainLooprights, 4);
			} 
	  	else if(threeShots){
	      		console.log("Working button");
				rad = launcht2.value * Math.PI / 180;
				vox1 = -(launchv2.value * Math.cos(rad+0.15));
				voy1 = launchv2.value * Math.sin(rad+0.15);
				vox2 = -(launchv2.value * Math.cos(rad));
				voy2 = launchv2.value * Math.sin(rad);
				vox3 = -(launchv2.value * Math.cos(rad-0.15));
				voy3 = launchv2.value * Math.sin(rad-0.15);
